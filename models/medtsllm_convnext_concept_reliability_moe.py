"""MedTsLLM + ConvNeXt + concept-constrained reliability-aware semantic MoE routing.
...
from .concept_reliability_moe_components import (
    AttentionPool,
    ConceptReliabilitySemanticMoERouter,
    cfg_get,
    masked_query_consistency_loss,
    normalized_entropy,
    query_diversity_loss,
    reliability_supervision_loss,
...
class MedTsLLMConvNeXtConceptReliabilityMoE(MedTsLLM):
    """Concept-constrained reliability-aware sparse semantic MoE for ECG."""
    ...
    def __init__(self, config: Any, dataset: Any) -> None:
        ...
        section = cfg_get(self.model_config, "concept_reliability_moe", None)
        if section is None:
            raise ValueError(
                "Missing [models.medtsllm.concept_reliability_moe] in TOML config"
            )
        ...
        top_k = int(cfg_get(section, "top_k", 2))
        modality_dropout_p = float(cfg_get(section, "modality_dropout_p", 0.15))
        ...
        self.router = ConceptReliabilitySemanticMoERouter(
            dim=self.q_dim,
        ...
        self.loss_weights = {
            ...
            "concept_sparsity": float(cfg_get(loss_cfg, "concept_sparsity", 0.01)),
            "routing_sparsity": float(cfg_get(loss_cfg, "routing_sparsity", 0.01)),
            "reliability": float(cfg_get(loss_cfg, "reliability", 0.05)),
        ...
        self.last_shared_attention: Optional[list[Tensor]] = None
        self.last_concept_weights: Optional[Tensor] = None
        self.last_signal_reliability: Optional[Tensor] = None
        self.last_image_reliability: Optional[Tensor] = None
        self.last_expert_routing: Optional[Tensor] = None
    ...
    def _set_auxiliary_losses(
        ...
        both_available = signal_available & image_available
        raw = {
            "med_ce": zero,
            "image_ce": zero,
            "query_ce": zero,
            "query_consistency": masked_query_consistency_loss(
                signal_evidence, image_evidence, both_available
            ),
            "query_diversity": query_diversity_loss(queries),
            "semantic_anchor": semantic_anchor_loss(adapted_queries, structured_queries),
            # Minimizing normalized entropy encourages a small number of clinical
            # concepts / experts to dominate, improving interpretability.
            "concept_sparsity": normalized_entropy(concept_weights),
            "routing_sparsity": normalized_entropy(routing_probs),
            "reliability": reliability_supervision_loss(
                signal_reliability, image_reliability,
                signal_available, image_available,
            ),
            "biomedcoop": bc_aux,
        }

        if labels is not None:
            # Aux branch losses are evaluated only where that latent modality was
            # kept, so synthetic modality dropout does not train a branch on zeros.
            med_logits = self.med_aux_head(med_repr)
            img_logits = self.image_aux_head(image_repr)
            if signal_available.any():
                raw["med_ce"] = self._branch_loss(
                    med_logits[signal_available], labels[signal_available]
                )
            if image_available.any():
                raw["image_ce"] = self._branch_loss(
                    img_logits[image_available], labels[image_available]
                )
    ...
    def forward(self, inputs: dict[str, Any]) -> Tensor:
        ...
        concept_weights = diagnostics["concept_weights"]
        routing_probs = diagnostics["routing_probs"]
        signal_reliability = diagnostics["signal_reliability"]
        image_reliability = diagnostics["image_reliability"]
        signal_available = diagnostics["signal_available"]
        image_available = diagnostics["image_available"]
        ...
        assert isinstance(concept_weights, Tensor)
        assert isinstance(routing_probs, Tensor)
        assert isinstance(signal_reliability, Tensor)
        assert isinstance(image_reliability, Tensor)
        assert isinstance(signal_available, Tensor)
        assert isinstance(image_available, Tensor)
        self._set_auxiliary_losses(
            med_repr, image_repr, query_repr, queries,
            adapted_queries, structured_queries,
            signal_evidence, image_evidence, concept_weights, routing_probs,
            signal_reliability, image_reliability, signal_available,
            image_available, labels,
        )
        ...
        self.last_shared_attention = [x.detach() for x in diagnostics["shared_attention"]]  # type: ignore[arg-type]
        self.last_concept_weights = concept_weights.detach()
        self.last_signal_reliability = signal_reliability.detach()
        self.last_image_reliability = image_reliability.detach()
        self.last_expert_routing = routing_probs.detach()
        return logits
    ...
    def train(self, mode: bool = True) -> "MedTsLLMConvNeXtConceptReliabilityMoE":
        super().train(mode)
