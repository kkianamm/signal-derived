...
class MedTsLLM(nn.Module):
    ...
    def pad_sequence(self, seq, seq_len):
        """Left-pad `seq` to `seq_len` along dim=1 with the tokenizer's PAD embedding.

        Returns (padded_seq, mask) where mask is [bs, seq_len] with 1 for real
        (non-pad) positions and 0 for the padding block, so callers can build a
        proper attention_mask instead of silently attending over left-padding
        ...
        bs, cur_len = seq.size(0), seq.size(1)
        if cur_len < seq_len:
            pad_len = seq_len - cur_len
            pad = pad_embedding.unsqueeze(0).expand(bs, pad_len, -1)
            seq = torch.cat([pad, seq], dim=1)
            mask = torch.cat([
                torch.zeros(bs, pad_len, device=seq.device, dtype=torch.long),
                torch.ones(bs, cur_len, device=seq.device, dtype=torch.long),
            ], dim=1)
        else:
            mask = torch.ones(bs, cur_len, device=seq.device, dtype=torch.long)
        return seq, mask
    ...
    def predict(self, inputs):
        ...
        if len(prompts[0]) > 0:
            ...
            padded = [self.pad_sequence(enc, max_len) for enc in prompt_enc]

            prompt_enc = torch.cat([p[0] for p in padded], dim=0)
            prompt_mask = torch.cat([p[1] for p in padded], dim=0)   # [bs, max_len]: 0=pad, 1=real
        else:
            prompt_enc = torch.zeros((bs, 0, self.d_llm), device=x_enc.device, dtype=x_enc.dtype)
            prompt_mask = torch.ones((bs, 0), device=x_enc.device, dtype=torch.long)
        ...
        if self.covariate_mode == "independent" or self.covariate_mode == "merge-end":
            ...
            prompt_mask = prompt_mask.repeat_interleave(n_features, dim=0)

        # The time-series patch embeddings (x_enc) are never padded -- every
        # example has exactly n_patches valid positions -- so they need no mask
        # of their own; only the (variable-length, left-padded) text prompt does.
        if self.llm.config.is_encoder_decoder:
            dec_out = self.llm(
                inputs_embeds=prompt_enc,
                attention_mask=prompt_mask,
                decoder_inputs_embeds=x_enc,
            ).last_hidden_state  # [bs, n_patches, d_llm]
        else:
            enc = torch.cat([prompt_enc, x_enc], dim=1)
            ts_mask = torch.ones(x_enc.size(0), x_enc.size(1), device=x_enc.device, dtype=torch.long)
            combined_mask = torch.cat([prompt_mask, ts_mask], dim=1)
            dec_out = self.llm(inputs_embeds=enc, attention_mask=combined_mask).last_hidden_state  # [bs, n_tok + n_patches, d_llm]
